...
pub fn command() -> Command {
    Command::new(NAME)
        ...
        // only required for custom VM installation right after network creation
        .arg(
            Arg::new("VM_BINARY_FILE")
                .long("vm-binary-file")
                .help("VM binary local file path (only required for custom VM installation, must exist to auto-install subnet/chain)")
                .required(false)
                .num_args(1),
        )
        .arg(
        ...
        .arg(
            Arg::new("CHAIN_GENESIS_FILE")
                .long("chain-genesis-file")
                .help("Chain genesis file path (only required for custom VM installation)")
                .required(false)
                .num_args(1),
        )
        .arg(
            Arg::new("CHAIN_CONFIG_FILE")
                .long("chain-config-file")
                .help("Chain configuration local file path (only required for custom VM installation)")
                .required(false)
                .num_args(1),
        )
}
