...
const SUBCOMMAND_UPDATE_ARTIFACTS: &str = "update-artifacts";
...
fn create_update_artifacts_command() -> Command<'static> {
    Command::new(SUBCOMMAND_READ_SPEC)
...
fn main() {
    let matches = Command::new(APP_NAME)
        ...
        .subcommands(vec![
            ...
            create_update_artifacts_command(),
    ...
    match matches.subcommand() {
        ...
        Some((SUBCOMMAND_UPDATE_ARTIFACTS, sub_matches)) => {
            execute_update_artifacts_spec(
...
fn execute_update_artifacts_spec(
    log_level: &str,
    spec_file_path: &str,
    ...
    rt.block_on(
        s3_manager.put_object(
            ...
            &avalanche_ops::StorageNamespace::EventsUpdateArtifactsInstallDirAvalanchedBin(
                spec.id.clone(),
            )
            .encode(),
        ),
    )
    .expect("failed put_object install_artifacts_avalanched_bin");
