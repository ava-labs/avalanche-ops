...
impl Tx {
    ...
    pub fn type_id() -> io::Result<u32> {
        if let Some(type_id) = codec::WALLET_P_TYPES.get("platformvm.UnsignedCreateSubnetTx") {
            Ok((*type_id) as u32)
        } else {
...
/// RUST_LOG=debug cargo test --package avalanche-ops --lib -- avalanche::types::platformvm::create_subnet::test_create_subnet_tx_serialization_with_one_signer --exact --show-output
#[test]
fn test_create_subnet_tx_serialization_with_one_signer() {
    ...
    let test_key =
        key::Key::from_private_key("PrivateKey-ewoqjP7PxY4yr3iLTpLisriqt94hdyDFNgchSxGGztUrTXtNN")
...
/// RUST_LOG=debug cargo test --package avalanche-ops --lib -- avalanche::types::platformvm::create_subnet::test_create_subnet_tx_serialization_with_custom_network --exact --show-output
#[test]
