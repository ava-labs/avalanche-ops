...
use crate::{
    avalanche::types::{avax, codec, ids, key, platformvm, secp256k1fx},
...
/// RUST_LOG=debug cargo test --package avalanche-ops --lib -- avalanche::types::platformvm::add_subnet_validator::test_add_subnet_validator_tx_serialization_with_one_signer --exact --show-output
#[test]
fn test_add_subnet_validator_tx_serialization_with_one_signer() {
    ...
    let mut tx = Tx {
        ...
        validator: Validator {
            ...
            subnet_id: ids::Id::new(&<Vec<u8>>::from([
    ...
    let keys1: Vec<key::Key> = vec![test_key.clone()];
    let keys2: Vec<key::Key> = vec![test_key];
