...
impl Tx {
    ...
    pub fn type_id() -> io::Result<u32> {
        if let Some(type_id) = codec::WALLET_P_TYPES.get("platformvm.UnsignedCreateChainTx") {
            Ok((*type_id) as u32)
        } else {
...
/// RUST_LOG=debug cargo test --package avalanche-ops --lib -- avalanche::types::platformvm::create_chain::test_create_chain_tx_serialization_with_one_signer --exact --show-output
#[test]
