...
use crate::{
    avalanche::types::{avax, avm::fx, codec, ids, key, secp256k1fx},
...
impl Tx {
    ...
    pub fn type_id() -> io::Result<u32> {
        if let Some(type_id) = codec::WALLET_X_TYPES.get("avm.ImportTx") {
            Ok((*type_id) as u32)
        } else {
...
/// RUST_LOG=debug cargo test --package avalanche-ops --lib -- avalanche::types::avm::import::test_import_tx_serialization_with_no_signer --exact --show-output
...
/// RUST_LOG=debug cargo test --package avalanche-ops --lib -- avalanche::types::avm::import::test_import_tx_serialization_with_two_signers --exact --show-output
/// ref. "avalanchego/vms/avm.TestImportTxSerialization"
#[test]
