...
pub fn from_zstd(d: &[u8]) -> io::Result<Vec<u8>> {
    ...
    let decompressed = zstd::stream::decode_all(Cursor::new(d))?;
...
pub fn to_gzip(d: &[u8]) -> io::Result<Vec<u8>> {
    let size = d.len() as f64;
    info!(
        "compressing to gzip (current size {})",
        humanize::bytes(size),
    );
    ...
    let size = compressed.len() as f64;
    info!("compressed to gzip (new size {})", humanize::bytes(size),);

    Ok(compressed)
}

pub fn from_gzip(d: &[u8]) -> io::Result<Vec<u8>> {
    let size = d.len() as f64;
    info!(
        "decompressing gzip (current size {})",
        humanize::bytes(size),
    );
    ...
    let size = decompressed.len() as f64;
    info!("decompressed gzip (new size {})", humanize::bytes(size),);
...
pub fn to_zstd_file(src_path: &str, dst_path: &str, level: Option<i32>) -> io::Result<()> {
    ...
    info!(
        "compressing '{}' to '{}' with zstd (current size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
    ...
    let size = meta.len() as f64;
    info!(
        "compressed '{}' to '{}' with zstd (new size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
...
pub fn from_zstd_file(src_path: &str, dst_path: &str) -> io::Result<()> {
    ...
    let size = meta.len() as f64;
    info!(
        "decompressing '{}' to '{}' with zstd (current size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
    ...
    let size = meta.len() as f64;
    info!(
        "decompressed '{}' to '{}' with zstd (new size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
...
/// Compresses the contents in "src_path" using "Gzip" compression
/// and saves it to "dst_path". Note that even if "dst_path" already exists,
/// it truncates (overwrites).
pub fn to_gzip_file(src_path: &str, dst_path: &str) -> io::Result<()> {
    let meta = fs::metadata(src_path)?;
    let size = meta.len() as f64;
    info!(
        "compressing '{}' to '{}' with gzip (current size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
    ...
    let size = meta.len() as f64;
    info!(
        "compressed '{}' to '{}' with gzip (new size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
...
/// Decompresses the contents in "src_path" using "Gzip" and saves it
/// to "dst_path". Note that even if "dst_path" already exists, it truncates
/// (overwrites).
pub fn from_gzip_file(src_path: &str, dst_path: &str) -> io::Result<()> {
    let meta = fs::metadata(src_path)?;
    let size = meta.len() as f64;
    info!(
        "decompressing '{}' to '{}' with gzip (current size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
    ...
    let size = meta.len() as f64;
    info!(
        "decompressed '{}' to '{}' with gzip (new size {})",
        src_path,
        dst_path,
        humanize::bytes(size),
    );
...
fn test_compress_gzip() {
    ...
    let contents = random::string(10);
    let contents_compressed = to_gzip(&contents.as_bytes()).unwrap();
    let contents_compressed_decompressed = from_gzip(&contents_compressed).unwrap();
    assert_eq!(contents.as_bytes(), contents_compressed_decompressed);
    info!("contents_compressed: {:#?}", contents_compressed);
...
/// Archives the source directory "src_dir_path" with zip and gzip compression
/// and saves to "dst_path". If "dst_path" exists, it overwrites.
pub fn to_zip_gzip(src_dir_path: &str, dst_path: &str) -> io::Result<()> {
    let size = match get_size(src_dir_path) {
        Ok(v) => v,
        Err(e) => {
            return Err(Error::new(
    ...
    let size = size as f64;
    info!(
        "start to_zip_gzip directory '{}' (original size {}) to '{}'",
        src_dir_path,
        humanize::bytes(size),
        dst_path
    );
    ...
    zip.finish()?;
    info!("wrote zip file {}", zip_path);
    to_gzip_file(&zip_path, dst_path)?;

    let meta = fs::metadata(dst_path)?;
    let size = meta.len() as f64;
    info!(
        "done to_zip_gzip '{}' to '{}' (final zipped, compressed size {})",
        src_dir_path,
        dst_path,
        humanize::bytes(size)
    );
...
/// Archives the source directory "src_dir_path" with tar and gzip compression
/// and saves to "dst_path". If "dst_path" exists, it overwrites.
///
/// e.g.,
...
///
pub fn to_tar_gzip(src_dir_path: &str, dst_path: &str) -> io::Result<()> {
    let size = match get_size(src_dir_path) {
        Ok(v) => v,
        Err(e) => {
            return Err(Error::new(
    ...
    let size = size as f64;
    info!(
        "start to_tar_gzip directory '{}' (original size {}) to '{}'",
        src_dir_path,
        humanize::bytes(size),
        dst_path
    );
    ...
    info!("wrote tar file {}", tar_path);
    to_gzip_file(&tar_path, dst_path)?;

    let meta = fs::metadata(dst_path)?;
    let size = meta.len() as f64;
    info!(
        "done to_tar_gzip '{}' to '{}' (final tar-ed, compressed size {})",
        src_dir_path,
        dst_path,
        humanize::bytes(size)
    );
...
/// Un-archives the zip file with "gzip" and unpack them to the destination
/// directory "dst_dir_path".
pub fn from_zip_gzip(src_archive_path: &str, dst_dir_path: &str) -> io::Result<()> {
    let meta = fs::metadata(src_archive_path)?;
    let size = meta.len() as f64;
    info!(
        "start from_zip_gzip archive '{}' (original size {}) to '{}'",
        src_archive_path,
        humanize::bytes(size),
        dst_dir_path
    );
    ...
    let zip_path = random::tmp_path(10)?;
    from_gzip_file(src_archive_path, &zip_path)?;
    ...
    let size = size as f64;
    info!(
        "done from_zip_gzip '{}' to '{}' (final decompressed, unzipped size {})",
        src_archive_path,
        dst_dir_path.display(),
        humanize::bytes(size)
    );
...
/// Un-archives the tar file with "gzip" and unpack them to the destination
/// directory "dst_dir_path".
pub fn from_tar_gzip(src_archive_path: &str, dst_dir_path: &str) -> io::Result<()> {
    let meta = fs::metadata(src_archive_path)?;
    let size = meta.len() as f64;
    info!(
        "start from_tar_gzip archive '{}' (original size {}) to '{}'",
        src_archive_path,
        humanize::bytes(size),
        dst_dir_path
    );
    ...
    let tar_path = random::tmp_path(10)?;
    from_gzip_file(src_archive_path, &tar_path)?;
    ...
    let size = size as f64;
    info!(
        "done from_tar_gzip '{}' to '{}' (final decompressed, un-tar-ed size {})",
        src_archive_path,
        dst_dir_path.display(),
        humanize::bytes(size)
    );
...
fn test_archive_zip_gzip() {
    ...
    let zip_path = random::tmp_path(10).unwrap();
    to_zip_gzip(src_dir_path.as_os_str().to_str().unwrap(), &zip_path).unwrap();
    let dst_dir_path2 = env::temp_dir().join(random::string(10));
    let dst_dir_path2 = dst_dir_path2.as_os_str().to_str().unwrap();
    from_zip_gzip(&zip_path, dst_dir_path2).unwrap();

    let tar_path = random::tmp_path(10).unwrap();
    to_tar_gzip(src_dir_path.as_os_str().to_str().unwrap(), &tar_path).unwrap();
    let dst_dir_path2 = env::temp_dir().join(random::string(10));
    let dst_dir_path2 = dst_dir_path2.as_os_str().to_str().unwrap();
    from_tar_gzip(&tar_path, dst_dir_path2).unwrap();
}
