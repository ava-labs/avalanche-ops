...
fn main() {
    ...
    let (node_id1, generated1) =
        avalanche_types::cert::x509::load_or_generate_pem(&tls_key_path1, &tls_cert_path1).unwrap();
    ...
    let s3_key_tls_key = format!("{}.key.zstd.encrypted", node_id1);
    let s3_key_tls_cert = format!("{}.crt.zstd.encrypted", node_id1);
    ab!(certs_manager.upload(
        &tls_key_path1,
        &tls_cert_path1,
        &s3_key_tls_key,
        &s3_key_tls_cert
    ...
    let node_id3 = ab!(certs_manager.download(
        &s3_key_tls_key,
        &s3_key_tls_cert,
        &tls_key_path2,
        &tls_cert_path2
