...
pub fn execute(log_level: &str, spec_file_path: &str, skip_prompt: bool) -> io::Result<()> {
    ...
    let mut spec = dev_machine_aws::Spec::load(spec_file_path).unwrap();
    spec.validate()?;
    ...
    if aws_resources.cloudformation_ec2_instance_role.is_none() {
        aws_resources.cloudformation_ec2_instance_role =
            Some(dev_machine_aws::StackName::Ec2InstanceRole(spec.id.clone()).encode());
    }
    if aws_resources.cloudformation_vpc.is_none() {
        aws_resources.cloudformation_vpc =
            Some(dev_machine_aws::StackName::Vpc(spec.id.clone()).encode());
    }
    if aws_resources.cloudformation_asg.is_none() {
        aws_resources.cloudformation_asg =
            Some(dev_machine_aws::StackName::Asg(spec.id.clone()).encode());
    }
    ...
    rt.block_on(s3_manager.put_object(
        ...
        Arc::new(dev_machine_aws::StorageNamespace::DevMachineConfigFile(spec.id.clone()).encode()),
    ))
    .unwrap();
    ...
    if aws_resources.kms_cmk_id.is_none() && aws_resources.kms_cmk_arn.is_none() {
        ...
        rt.block_on(s3_manager.put_object(
            ...
            Arc::new(aws_resources.bucket.clone()),
            Arc::new(
                dev_machine_aws::StorageNamespace::DevMachineConfigFile(spec.id.clone()).encode(),
            ),
        ))
        .unwrap();
    }
    ...
    if aws_resources.ec2_key_path.is_none() {
        ...
        rt.block_on(
            s3_manager.put_object(
                ...
                Arc::new(aws_resources.bucket.clone()),
                Arc::new(
                    dev_machine_aws::StorageNamespace::Ec2AccessKeyCompressedEncrypted(
                        spec.id.clone(),
                    )
                    .encode(),
                ),
...
",
        ...
        rt.block_on(s3_manager.put_object(
            ...
            Arc::new(dev_machine_aws::StorageNamespace::DevMachineConfigFile(spec.id).encode()),
        ))
        .unwrap();
